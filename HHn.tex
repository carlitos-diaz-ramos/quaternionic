...
\]
Thus, we rescale $\mathcal{B}_\theta$ so that $\phi_{*o}\colon\g{p}\to T_o\mathsf{H}_r(V)$ is a homothecy, and set
\[
\langle X,Y\rangle_{\g{g}}
=\frac{c}{4(n+1)}\mathcal{B}_\theta(X,Y)
=c\Re\tr_\H(X^*Y),
\]
for $X$, $Y\in\g{g}$, and $c>0$.
